    ...
    try {
        ...
        if ($verStr -match "Python (\d+)\.(\d+)") {
            ...
            if ($maj -gt 3 -or ($maj -eq 3 -and $min -ge 11)) {
                ...
                if (-not $python) { $python = $cmd }
                Write-Ok "$verStr  ->  $python"
                break
            }
        }
    } catch {}
...
if (Test-Path ".env") {
    ...
    if (Test-Path ".env.example") {
        ...
        Write-Host ""
        Write-Host "  !!! Mo file .env va dien credentials truoc khi tiep tuc:" -ForegroundColor Yellow
...
Set-Content -Path $profilePath -Encoding utf8 -Value @"
supply_chain_dbt:
  target: dev
  outputs:
    dev:
...
if (-not $supabaseDbUrl) {
    ...
    # Ghi ra file .py tam thay vi pipe vao stdin (tranh loi "output stream redirected")
    $tmpPy = Join-Path $env:TEMP "sc_supabase_init.py"
